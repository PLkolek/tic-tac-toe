...
export const getColumn = (board: Board, x: BoardCoordinate): Line =>
    board.map((row) => row[x]) as Line
...
export const getAllColumns = (board: Board): Line[] =>
    boardCoordinates.map((x) => getColumn(board, x))
